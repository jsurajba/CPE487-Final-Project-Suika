...
----
---- Target: Nexys A7-100T (Artix-7 XC7A100T-1CSG324C)
---- Display: 800x600 @ 60 Hz, 40 MHz pixel clock (matches user's vga_sync.vhd)
---- Fixed-point format: Q12.4 signed (16-bit, lower 4 bits fractional)
----   1 pixel = 16 internal units. Range +/- 2048 px (plenty).
----==============================================================================
--library ieee;
...
--    -- Game-over line: if a fruit's TOP is above this for too long, game over
--    constant GAMEOVER_LINE : integer := 80;

--    ----------------------------------------------------------------------------
--    -- Game parameters
--    ----------------------------------------------------------------------------
--    constant MAX_FRUITS : integer := 50;
...
--    -- Collision separation push (fraction of overlap vector applied each frame)
--    -- Smaller = softer/squishier; larger = stiffer.  Right-shift amount.
--    constant SEP_SHIFT  : integer := 4;   -- push by overlap_vec / 8
...
--        x"F22",  -- cherry      red
--        x"F88",  -- strawberry  pink-red
--        x"82F",  -- grape       purple
--        x"FA0",  -- dekopon     orange
--        x"F60",  -- persimmon   dark orange
--        x"F44",  -- apple       red
--        x"DC8",  -- pear        yellow-green
--        x"FBC",  -- peach       peach
--        x"FE4",  -- pineapple   yellow
--        x"4F4",  -- melon       green
--        x"2A4"   -- watermelon  dark green
--    );
...
--    ----------------------------------------------------------------------------
--    function fp_clamp(v, lo, hi : fixed_t) return fixed_t;
--    function fp_abs(v : fixed_t) return fixed_t;
...
--    function fp_clamp(v, lo, hi : fixed_t) return fixed_t is
--    begin
...
--    function fp_abs(v : fixed_t) return fixed_t is
--    begin
...
--Grok optimized
--==============================================================================
-- suika_pkg.vhd  (OPTIMIZED)
-- Shared types, constants, and helper functions for the Suika game.
--
-- CHANGES FOR FASTER SYNTHESIS:
--   - MAX_FRUITS reduced from 50 → 28 (huge reduction in renderer + physics logic)
--   - Removed unused fp_clamp and fp_abs functions
